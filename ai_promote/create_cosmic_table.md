...
10. **数据组 (`数据组`)**
    ...
    *   **唯一性** 数据组不能有重复，在表格里每行的数据组必须是唯一的。
    *   **多样性** 在整个表格内，每行的数据组尽量生成与其它行相似度**低**、语义**不同**的内容

11. **数据属性规则 (`数据属性`)**
    *   **数量要求”** 必须选择 **3-20个** 关键业务属性。
    *   必须为 **中文业务术语**，禁止英文或数据库字段名。
    *   用**中文逗号（，）** 分割。
    *   **唯一性**：数据属性不能有重复，在表格里每行的数据属性必须是唯一的。**即使是ERX结构的，R行和X行的数据属性也不能相同。**
