...
def validate_cosmic_table(markdown_table_str: str) -> Tuple[bool, str]:
    ...
    if headers_match:
        for row_info in table_rows_with_index:
            ...
            attributes = [attr.strip() for attr in attributes_str.split(ATTR_SEPARATOR) if attr.strip()]
            ...
            # 检查分隔符 (强制)
            if ATTR_SEPARATOR not in attributes_str and attr_count > 1:
                 # 错误提示：未使用中文逗号分隔
                 final_errors.append(f"数据属性格式错误 (第 {row_num} 行): 多个属性之间未使用中文逗号 '{ATTR_SEPARATOR}' 分隔。请使用中文逗号分隔。")
